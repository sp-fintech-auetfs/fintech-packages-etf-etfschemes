...
namespace Apps\Fintech\Packages\Etf\Schemes;
...
use Apps\Fintech\Packages\Etf\Schemes\Settings;
use Apps\Fintech\Packages\Etf\Tools\Extractdata\EtfToolsExtractdata;
use Apps\Fintech\Packages\Etf\Tools\Patterns\EtfToolsPatterns;
use League\Flysystem\FilesystemException;
use League\Flysystem\UnableToWriteFile;
use System\Base\BasePackage;

class EtfSchemes extends BasePackage
{
    protected $modelToUse = AppsFintechEtfSchemes::class;

    protected $packageName = 'etfschemes';
    ...
    public function getSchemeById(
        ...
        if (isset($this->schemes[$id])) {
            if (!$includeNavs) {
                if (isset($this->schemes[$id]['navs'])) {
                    unset($this->schemes[$id]['navs']);
                }
            }

            if (!$includeNavsChunks) {
                if (isset($this->schemes[$id]['navs_chunks'])) {
                    unset($this->schemes[$id]['navs_chunks']);
                }
            }

            if (!$includeNavsRR) {
                if (isset($this->schemes[$id]['rolling_returns'])) {
                    unset($this->schemes[$id]['rolling_returns']);
                }
            }

            // if (!$includeCustomNavs) {
            //     if (isset($this->schemes[$id]['custom'])) {
            //         unset($this->schemes[$id]['custom']);
            //     }
            // }

            // if (!$includeCustomNavsChunks) {
            //     if (isset($this->schemes[$id]['custom_chunks'])) {
            //         unset($this->schemes[$id]['custom_chunks']);
            //     }
            // }

            // if (!$includeCustomNavsRR) {
            //     if (isset($this->schemes[$id]['custom_rolling_returns'])) {
    ...
    public function getSchemeSnapshotById($id, $timeline, $includeNavsChunks = false, $includeNavsRR = false)
    {
        ...
        try {
            if ($this->localContent->fileExists('.ff/sp/apps_fintech_etf_schemes_snapshots/data/' . $id . '.json')) {
                $schemeSnapshot = $this->helper->decode($this->localContent->read('.ff/sp/apps_fintech_etf_schemes_snapshots/data/' . $id . '.json'), true);
            }
        ...
        if (!isset($schemeSnapshot['snapshots'][$timeline]) ||
            ...
            if (count($navs) > 0) {
                ...
                $etfToolsExtractDataPackage->createChunks($dbNav, ['get_all_navs' => 'true'], false, $schemeSnapshot);
                $etfToolsExtractDataPackage->createRollingReturns($dbNav, $id, ['get_all_navs' => 'true'], false, $schemeSnapshot);

                try {
                    $this->localContent->write('.ff/sp/apps_fintech_etf_schemes_snapshots/data/' . $id . '.json', $this->helper->encode($schemeSnapshot));
        ...
        if ($includeNavsChunks || $includeNavsRR) {
            ...
            if ($includeNavsChunks) {
                try {
                    if ($this->localContent->fileExists('.ff/sp/apps_fintech_etf_schemes_snapshots_navs_chunks/data/' . $schemeSnapshot['navs_chunks_ids'][$timeline] . '.json')) {
                        $scheme['navs_chunks'] = $this->helper->decode($this->localContent->read('.ff/sp/apps_fintech_etf_schemes_snapshots_navs_chunks/data/' . $schemeSnapshot['navs_chunks_ids'][$timeline] . '.json'), true);
                    }
                } catch (FilesystemException | UnableToReadFile | UnableToCheckExistence | \throwable $e) {
                    $this->addResponse($e->getMessage(), 1);
            ...
            if ($includeNavsRR) {
                try {
                    if ($this->localContent->fileExists('.ff/sp/apps_fintech_etf_schemes_snapshots_navs_rolling_returns/data/' . $schemeSnapshot['rolling_returns_ids'][$timeline] . '.json')) {
                        $scheme['rolling_returns'] = $this->helper->decode($this->localContent->read('.ff/sp/apps_fintech_etf_schemes_snapshots_navs_rolling_returns/data/' . $schemeSnapshot['rolling_returns_ids'][$timeline] . '.json'), true);
                    }
                } catch (FilesystemException | UnableToReadFile | UnableToCheckExistence | \throwable $e) {
                    $this->addResponse($e->getMessage(), 1);
    ...
    public function getEtfTypeBySymbol($symbol)
    {
        ...
        $etfscheme = $this->getByParams($conditions);

        if ($etfscheme && count($etfscheme) > 0) {
            return $etfscheme[0];
        }
    ...
    public function addEtfSchemes($data)
    {
        //
    }

    public function updateEtfSchemes($data)
    {
        //
    }

    public function removeEtfSchemes($data)
    {
        //
    }
    ...
    public function getSchemeInfo($data)
    {
        ...
        try {
            ...
            if ($scheme) {
                ...
                $data['symbol'] = $scheme['symbol'];

                $etfExtractdataPackage = $this->usePackage(EtfToolsExtractdata::class);

                $remoteScheme = $etfExtractdataPackage->sync($data);

                $details = [];

                if ($remoteScheme && count($remoteScheme) > 0) {
                    $details['aetfi_code'] = $scheme['aetfi_code'];
                ...
                if (count($details) > 0) {
                    ...
                    $detailsModel = new AppsFintechEtfSchemesDetails;

                    if ($this->config->databasetype === 'db') {
                        $dbDetails = $detailsModel::findFirst(['aetfi_code = ' . $scheme['aetfi_code']]);
                    } else {
                        $detailsStore = $this->ff->store($detailsModel->getSource());

                        $dbDetails = $detailsStore->findOneBy(['aetfi_code', '=', $scheme['aetfi_code']]);
                    }
        ...
        if ($schemesArr && count($schemesArr) > 0) {
            ...
            foreach ($schemesArr as $scheme) {
                if (str_contains(strtolower($scheme['name']), strtolower($data['search']))) {
                    ...
                    $schemes[$scheme['id']]['name'] = $scheme['name'] . ' (' . $scheme['symbol'] . ')';
                    $schemes[$scheme['id']]['symbol'] = $scheme['symbol'];
                }
            }
    ...
    public function getSchemeFromEtfCodeOrSchemeId(&$data, $includeNavs = false, $includeNavsChunks = false, $includeNavsRR = false)
    {
        if (isset($data['scheme_id']) && $data['scheme_id'] !== '') {
            $schemeId = (int) $data['scheme_id'];
        } else if (isset($data['aetfi_code']) && $data['aetfi_code'] !== '') {
            $schemeId = (int) $data['aetfi_code'];
        }
    ...
    {
        if ($customChunks) {
            $scheme = $this->getSchemeById((int) $this->access->auth->account()['id'] . $data['scheme_id'], false, true, false, true);

            if (!isset($scheme['navs_chunks']['navs_chunks'])) {
                ...
                if (count($responseData['chunks']) > 0) {
                    ...
                    foreach ($responseData['chunks'][$data['chunk_size']] as $chunkDate => $chunk) {
                        ...
                        if (isset($chunk['dividends_amount']) && $chunk['dividends_amount'] > 0) {
                            ...
                            $schemeDividends += $chunk['dividends_amount'];
                        }
                        ...
                        $responseData['chunks'][$data['chunk_size']][$chunkDate]['nav_with_div'] =
                            ...
                            $timeDateChunkNav = $chunk['nav'] + $schemeDividends;
                        ...
                        $responseData['chunks'][$data['chunk_size']][$chunkDate]['reinvest_diff_percent'] =
                            ...
                            round((($timeDateChunkNav - $firstNav) / $firstNav) * 100, 2);
                    }
    ...
    public function getSchemeRollingReturns($data, $customRollingReturns = false, $timeline = false)
    {
        ...
        if ($customRollingReturns) {
            $scheme = $this->getSchemeById((int) $this->access->auth->account()['id'] . $data['scheme_id'], false, false, true, true);
        ...
        try {
            ...
            if ($data['rr_period'] === 'year') {
                ...
                $endDate->subYear(25);
            } else if ($data['rr_period'] === 'thirty_year') {
                $endDate->subYear(30);
            }
    ...
    public function generateCustomNav($data)
    {
        ...
        if ($data['source'] === 'fixed_linear' || $data['source'] === 'fixed_random') {
            ...
            $etfToolsPatternsPackage = $this->usePackage(EtfToolsPatterns::class);

            $etfToolsPattern = $etfToolsPatternsPackage->getById((int) $data['pattern_id']);

            if (!$etfToolsPattern || $etfToolsPattern && !isset($etfToolsPattern['pattern'])) {
                $this->addResponse('Please provide correct pattern ID', 1);

                return;
            }

            for ($numberOfDays = 0; $numberOfDays < count($etfToolsPattern['pattern']); $numberOfDays++) {
                if (isset($etfToolsPattern['pattern'][$numberOfDays + 1])) {
                    $patterns[$numberOfDays + 1] = numberFormatPrecision($etfToolsPattern['pattern'][$numberOfDays + 1], 2);
    ...
    public function updateSchemeCustomNavs($data)
    {
        ...
        //Increase Exectimeout to 10mins as this process takes time to extract and merge data.
        ...
        $customScheme['id'] = $this->access->auth->account()['id'] . $scheme['id'];
        $customScheme['name'] = $scheme['name'] . ' (' . $this->access->auth->account()['username'] . ')';
        $customScheme['account_id'] = $this->access->auth->account()['id'];
        $customScheme['category_id'] = $scheme['category_id'];
        $customScheme['amc_id'] = $scheme['amc_id'];
        $customScheme['launch_date'] = $scheme['launch_date'];
        $customScheme['start_date'] = $scheme['start_date'];
        $customScheme['navs_last_updated'] = $this->helper->lastKey($customNavs);
        $customScheme['latest_nav'] = $this->helper->last($customNavs)['nav'];
        $customScheme['shared_ids'] = $this->helper->encode([]);

        try {
            $this->localContent->write('.ff/sp/apps_fintech_etf_schemes_custom/data/' . $customScheme['id'] . '.json', $this->helper->encode($customScheme));
        } catch (FilesystemException | UnableToWriteFile | \throwable $e) {
            $this->addResponse($e->getMessage(), 1);
        ...
        try {
            $this->localContent->write('.ff/sp/apps_fintech_etf_schemes_custom_navs/data/' . $customScheme['id'] . '.json', $this->helper->encode(
                [
                    'id'            => $customScheme['id'],
        ...
        try {
            $this->localContent->write('.ff/sp/apps_fintech_etf_schemes_custom_navs_chunks/data/' . $chunks['id'] . '.json', $this->helper->encode($chunks));
        } catch (FilesystemException | UnableToWriteFile | \throwable $e) {
            $this->addResponse($e->getMessage(), 1);
        ...
        //RollingReturns
        $rr = [];
        $rr['id'] = (int) $customScheme['id'];
        $rr['last_updated'] = $this->helper->lastKey($customNavs);

        $schemeRr = [];
        $schemeRr['day_cagr'] = $this->helper->last($customNavs)['diff_percent'];
        $schemeRr['day_trajectory'] = $this->helper->last($customNavs)['trajectory'];
        ...
        $customScheme = array_replace($customScheme, $schemeRr);
        try {
            $this->localContent->write('.ff/sp/apps_fintech_etf_schemes_custom/data/' . $customScheme['id'] . '.json', $this->helper->encode($customScheme));
        } catch (FilesystemException | UnableToWriteFile | \throwable $e) {
            $this->addResponse($e->getMessage(), 1);
        ...
        if (!isset($customNavs[$yearBefore])) {
            try {
                $this->localContent->write('.ff/sp/apps_fintech_etf_schemes_custom_navs_rolling_returns/data/' . $rr['id'] . '.json', $this->helper->encode($rr));
            } catch (FilesystemException | UnableToWriteFile | \throwable $e) {
                $this->addResponse($e->getMessage(), 1);
            ...
            foreach (['year', 'two_year', 'three_year', 'five_year', 'seven_year', 'ten_year', 'fifteen_year', 'twenty_year', 'twenty_five_year', 'thirty_year'] as $rrTerm) {
                try {
                    ...
                    if (isset($customNavs[$toDate])) {
                        ...
                        if ($toDate === $this->helper->last($customNavs)['date']) {
        ...
        $this->addResponse('Generated Custom Navs for scheme ' . $customScheme['name']);
    }
    ...
    public function getWatchlistByAccountId()
    {
        ...
        if ($this->config->databasetype === 'db') {
            ...
            $watchlists = $this->getByParams(['conditions' => ['account_id', '=', $this->access->auth->account()['id']]]);
        }

        if ($watchlists && isset($watchlists[0])) {
            return $watchlists[0];
        }
    ...
    public function toggleWatchlist($schemeId)
    {
        $watchlists = $this->getWatchlistByAccountId();

        if (!$watchlists) {
            $watchlist['account_id'] = $this->access->auth->account()['id'];
            ...
            $this->addResponse('Unable to add to watchlist', 1);

            return false;
        } else {
            if (isset($watchlists['schemes'])) {
                if (in_array($schemeId, $watchlists['schemes'])) {
                    $key = array_search($schemeId, $watchlists['schemes']);

                    unset($watchlists['schemes'][$key]);

                    if ($this->update($watchlists)) {
                        $this->addResponse('Removed from watchlist', 0, ['task' => 'remove']);
